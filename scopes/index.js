 ...
 * JS is a compiled language as when there is any syntax error, it usually throws error without executing anything above/below it
 ...
 * Undefined
    - A variable is said to be undefined when it has been declared but havent been assigned any value to it in its current scope.
 * Undeclared
...
function compiled() {
	console.log('Run');
	// console.log()); // Here have added extra braces to generate error, so JS thows error without consoling the above code
}

function interpreted() {
	console.log('Run');
	test(); // Here have added unknown function, which runs code above it i.e the console, before throwing the error test is not defined.
}
